...
import { createVehicleUnavailability } from "@/features/unavailabilities/application/create-vehicle-unavailability";
...
export async function createVehicleUnavailabilityAction(
  ...
  const result = await createVehicleUnavailability(organizationSlug, parsed.data);
  ...
  revalidatePath(path);
  redirect(`${path}?resource=vehicles&feedback=created`);
}
