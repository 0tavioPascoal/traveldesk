...
import { changeVehicleUnavailabilityTypeStatus } from "@/features/unavailabilities/application/change-vehicle-unavailability-type-status";
...
export async function changeVehicleUnavailabilityTypeStatusAction(
  ...
  const result = await changeVehicleUnavailabilityTypeStatus(organizationSlug, parsed.data.id, parsed.data.active);
  ...
  revalidatePath(`/app/${organizationSlug}/cadastros/tipos-indisponibilidade/veiculos`);
