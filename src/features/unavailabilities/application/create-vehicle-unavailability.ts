...
export async function createVehicleUnavailability(
  ...
  const [resource, type, availability] = await Promise.all([
    supabase.from("vehicles").select("id").eq("organization_id", context.organization.id).eq("id", input.resourceId).eq("active", true).maybeSingle(),
    supabase.from("vehicle_unavailability_types").select("id").eq("organization_id", context.organization.id).eq("id", input.unavailabilityTypeId).eq("active", true).maybeSingle(),
    checkVehicleAvailability(organizationSlug, input.resourceId, period.startsAt, period.endsAt),
  ]);
  ...
  const { data, error } = await supabase.from("vehicle_unavailabilities").insert({
    organization_id: context.organization.id,
    vehicle_id: input.resourceId,
