...
export async function updateVehicleUnavailability(
  ...
  const currentResult = await supabase.from("vehicle_unavailabilities")
    .select("id, vehicle_id, unavailability_type_id, starts_at, ends_at, all_day, active")
  ...
  const resourceChanged = current.vehicle_id !== input.resourceId;
  ...
  if (typeChanged || (current.active && periodChanged)) {
    const { data, error } = await supabase.from("vehicle_unavailability_types").select("id")
  ...
  const { data, error } = await supabase.from("vehicle_unavailabilities").update({
