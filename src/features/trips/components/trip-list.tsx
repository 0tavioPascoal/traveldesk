...
export function TripList({
  ...
  return (
    <>
      <div className="hidden overflow-x-auto rounded-xl border border-border bg-card shadow-sm xl:block">
        <table className="w-full min-w-[1080px] text-left">
          ...
          <thead className="border-b border-border bg-muted/70 text-xs font-semibold uppercase tracking-wide text-muted-foreground">
            <tr>
              ...
              <th scope="col" className="px-4 py-3">Cliente e unidade</th>
              <th scope="col" className="px-4 py-3">Período</th>
              <th scope="col" className="px-4 py-3">Prioridade</th>
              <th scope="col" className="px-4 py-3">Status</th>
              <th scope="col" className="px-4 py-3">Próxima etapa</th>
            ...
            {items.map((item) => {
              ...
              return (
                <tr key={item.id} className={`transition-colors hover:bg-muted/45 focus-within:bg-muted/45 ${item.priority === "urgent" ? "border-l-2 border-l-destructive" : ""}`}>
                  ...
                  <td className="max-w-56 px-4 py-4 align-top text-sm">
                  ...
                  <td className="px-4 py-4 align-top"><TripPriorityBadge priority={item.priority} /></td>
                  <td className="px-4 py-4 align-top"><TripStatusBadge status={item.status} /></td>
                  <td className="max-w-40 px-4 py-4 align-top text-sm font-medium text-card-foreground">{tripNextStepLabels[item.status]}</td>
      ...
      <div className="space-y-3 xl:hidden">
        {items.map((item) => {
          const tripPeriod = period(item, timezone);
          return (
            <article key={item.id} className={`rounded-xl border bg-card p-4 shadow-sm ${item.priority === "urgent" ? "border-destructive/45" : "border-border"}`}>
              ...
              <p className="mt-3 text-sm text-card-foreground">{item.client_name_snapshot} <span aria-hidden="true" className="text-muted-foreground">·</span> {item.client_unit_name_snapshot}</p>
              <dl className="mt-4 grid gap-2 text-sm text-muted-foreground sm:grid-cols-2">
                <div className="flex items-start gap-2"><CalendarDays aria-hidden="true" className="mt-0.5 size-4 shrink-0" /><dd>{tripPeriod ? <>{tripPeriod.start}<span className="block">→ {tripPeriod.end}</span></> : "Período não informado"}</dd></div>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-border pt-3">
                <TripPriorityBadge priority={item.priority} />
                <span className="min-w-0 flex-1 text-sm font-medium text-card-foreground">{tripNextStepLabels[item.status]}</span>
