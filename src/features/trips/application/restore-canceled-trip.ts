...
export async function restoreCanceledTrip(organizationSlug: string, tripId: string): Promise<TripMutationResult> {
  const context = await requireOrganizationRole(organizationSlug, ["admin"] as const);
  const supabase = await createClient();
  const { data, error } = await supabase.rpc("restore_canceled_trip", {
