...
type Props = { organizationSlug: string; timezone: string; initialValues: TechnicianFormValues; skillOptions: Array<{ id: string; name: string; active: boolean }>; technicianId?: string };
const control = formControlClassName;
const states = ["AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG","PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO"];

export function TechnicianForm({ organizationSlug, timezone, initialValues, skillOptions, technicianId }: Props) {
  ...
  const field = (name: keyof TechnicianFormValues, label: string, attributes: React.InputHTMLAttributes<HTMLInputElement> = {}) => {
    ...
    return <div className="space-y-1.5"><label htmlFor={name} className="block text-sm font-medium text-foreground">{label} {attributes.required ? <RequiredIndicator /> : null}</label><input id={name} name={name} disabled={pending} defaultValue={String(state.values[name] ?? "")} aria-invalid={error ? true : undefined} aria-describedby={error ? errorId : undefined} className={control} {...attributes} />{error ? <p id={errorId} role="alert" className="text-sm text-destructive">{error}</p> : null}</div>;
  };

  return <form ref={formRef} action={formAction} noValidate onChange={() => setDirty(true)} className="space-y-6">
    <section aria-labelledby="personal-data-title" className={formSectionClassName}><SectionHeader id="personal-data-title" title="Informações pessoais" description="Dados de identificação e contato do técnico operacional." /><div className="mt-6 grid gap-5 sm:grid-cols-2 lg:grid-cols-3"><div className="sm:col-span-2 lg:col-span-3">{field("name", "Nome", { required: true, minLength: 2, maxLength: 160, autoFocus: true, autoComplete: "name" })}</div>{field("document", "CPF", { inputMode: "numeric", placeholder: "000.000.000-00" })}{field("email", "E-mail", { type: "email", maxLength: 254, autoComplete: "email" })}{field("phone", "Telefone", { inputMode: "tel", autoComplete: "tel", placeholder: "(00) 00000-0000" })}</div><p className="mt-4 text-xs leading-5 text-muted-foreground">O e-mail do técnico não cria nem vincula automaticamente um usuário de acesso.</p></section>

    <section aria-labelledby="professional-data-title" className={formSectionClassName}><SectionHeader id="professional-data-title" title="Dados profissionais" description="Informações usadas para identificar a função e as competências do técnico." /><div className="mt-6 max-w-3xl">{field("jobTitle", "Cargo ou função", { maxLength: 120 })}</div></section>

    <section aria-labelledby="base-location-title" className={formSectionClassName}><SectionHeader id="base-location-title" title="Localidade-base" description="Referência operacional usada no planejamento das viagens." /><div className="mt-6 grid gap-5 sm:grid-cols-[minmax(0,1fr)_10rem]">{field("baseCity", "Cidade-base", { required: true, maxLength: 120 })}<div className="space-y-1.5"><label htmlFor="baseState" className="block text-sm font-medium text-foreground">Estado-base <RequiredIndicator /></label><select id="baseState" name="baseState" required disabled={pending} defaultValue={state.values.baseState} aria-invalid={state.fieldErrors.baseState?.[0] ? true : undefined} aria-describedby={state.fieldErrors.baseState?.[0] ? "baseState-error" : undefined} className={control}><option value="">UF</option>{states.map((uf) => <option key={uf}>{uf}</option>)}</select>{state.fieldErrors.baseState?.[0] ? <p id="baseState-error" role="alert" className="text-sm text-destructive">{state.fieldErrors.baseState[0]}</p> : null}</div></div></section>
    ...
    <section aria-labelledby="driver-license-title" className={formSectionClassName}><SectionHeader id="driver-license-title" title="Habilitação" description="Autorização para conduzir veículos da empresa e dados da CNH." /><label className="mt-6 flex cursor-pointer items-start gap-3 rounded-xl border border-border bg-muted/50 p-4"><input type="checkbox" name="canDriveCompanyVehicle" checked={canDrive} disabled={pending} aria-controls="driver-license-fields" aria-expanded={canDrive} onChange={(event) => setCanDrive(event.target.checked)} className="mt-0.5 size-4" /><span><span className="block text-sm font-semibold text-foreground">Autorizado a dirigir veículos da empresa</span><span className="mt-1 block text-xs leading-5 text-muted-foreground">Quando autorizado, número, categoria e validade da CNH são obrigatórios.</span></span></label><div id="driver-license-fields" aria-hidden={!canDrive} className={`mt-5 grid gap-5 sm:grid-cols-3 ${canDrive ? "" : "hidden"}`}>{field("driverLicenseNumber", "Número da CNH", { inputMode: "numeric", maxLength: 14, required: canDrive })}<div className="space-y-1.5"><label htmlFor="driverLicenseCategory" className="block text-sm font-medium text-foreground">Categoria {canDrive ? <RequiredIndicator /> : null}</label><select id="driverLicenseCategory" name="driverLicenseCategory" disabled={pending} required={canDrive} defaultValue={state.values.driverLicenseCategory} aria-invalid={state.fieldErrors.driverLicenseCategory?.[0] ? true : undefined} aria-describedby={state.fieldErrors.driverLicenseCategory?.[0] ? "driverLicenseCategory-error" : undefined} className={control}><option value="">Selecione</option>{["A","B","C","D","E","AB","AC","AD","AE"].map((category) => <option key={category}>{category}</option>)}</select>{state.fieldErrors.driverLicenseCategory?.[0] ? <p id="driverLicenseCategory-error" role="alert" className="text-sm text-destructive">{state.fieldErrors.driverLicenseCategory[0]}</p> : null}</div><FormDateField id="driverLicenseExpiresAt" name="driverLicenseExpiresAt" label="Data de validade" defaultValue={state.values.driverLicenseExpiresAt} timezone={timezone} disabled={pending} required={canDrive} error={state.fieldErrors.driverLicenseExpiresAt?.[0]} /></div>{!canDrive ? <p className="mt-4 text-sm text-muted-foreground">Os dados de CNH existentes são preservados ao retirar a autorização, conforme o comportamento atual do domínio.</p> : null}</section>

    <section aria-labelledby="notes-title" className={formSectionClassName}><SectionHeader id="notes-title" title="Observações" description="Informações profissionais adicionais relevantes ao planejamento." /><div className="mt-6 space-y-1.5"><label htmlFor="notes" className="sr-only">Observações</label><textarea id="notes" name="notes" rows={5} maxLength={2000} disabled={pending} defaultValue={state.values.notes} onChange={(event) => setNotesLength(event.target.value.length)} aria-invalid={state.fieldErrors.notes?.[0] ? true : undefined} aria-describedby={`notes-help${state.fieldErrors.notes?.[0] ? " technician-notes-error" : ""}`} className={formTextareaClassName} /><div id="notes-help" className="flex justify-between gap-4 text-xs text-muted-foreground"><span>As quebras de linha serão preservadas.</span><span>{notesLength}/2.000</span></div>{state.fieldErrors.notes?.[0] ? <p id="technician-notes-error" role="alert" className="text-sm text-destructive">{state.fieldErrors.notes[0]}</p> : null}</div></section>

    {state.message ? <InlineAlert tone="error">{state.message}</InlineAlert> : null}
    <FormActions><FormCancelLink href={cancel} dirty={dirty} /><button type="submit" disabled={pending} className={buttonStyles()}>{pending ? "Salvando..." : technicianId ? "Salvar alterações" : "Criar técnico"}</button></FormActions>
  </form>;
}
