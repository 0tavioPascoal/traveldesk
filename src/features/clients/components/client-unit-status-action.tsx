...
export function ClientUnitStatusAction({ organizationSlug, clientId, unitId, active }: ClientUnitStatusActionProps) {
  ...
  if (active && !confirming) {
    return <button type="button" onClick={() => setConfirming(true)} className="text-sm font-medium text-red-700 hover:text-red-900">Inativar</button>;
  }
  ...
  if (active) {
    return (
      <form action={formAction} className="space-y-2 rounded-lg bg-red-50 p-2">
        ...
        {state.message ? <p role="alert" className="max-w-52 text-xs text-red-800">{state.message}</p> : null}
      </form>
    );
  }
  ...
  return (
    <form action={formAction} className="space-y-1">
      ...
      {state.message ? <p role={state.status === "error" ? "alert" : "status"} className="max-w-52 text-xs text-zinc-600">{state.message}</p> : null}
    </form>
  );
}
