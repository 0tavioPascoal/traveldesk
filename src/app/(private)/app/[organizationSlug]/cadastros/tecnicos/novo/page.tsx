...
export default async function NewTechnicianPage({ params }: { params: Promise<{ organizationSlug: string }> }) {
  ...
  return <PageContainer className="max-w-5xl space-y-6"><PageHeader title="Novo técnico" description="Cadastre os dados operacionais, a habilitação e as especialidades." breadcrumbs={[{ label: "Visão geral", href: `/app/${organizationSlug}/dashboard` }, { label: "Técnicos", href: `/app/${organizationSlug}/cadastros/tecnicos` }, { label: "Novo técnico" }]} /><TechnicianForm organizationSlug={organizationSlug} skillOptions={skills.map((skill) => ({ ...skill, active: true }))} initialValues={{ name: "", document: "", email: "", phone: "", jobTitle: "", baseCity: "", baseState: "", driverLicenseNumber: "", driverLicenseCategory: "", driverLicenseExpiresAt: "", canDriveCompanyVehicle: false, notes: "", skillAssignments: [] }} /></PageContainer>;
}
