...
export default async function EditTechnicianPage({ params }: { params: Promise<{ organizationSlug: string; technicianId: string }> }) {
  ...
  return <FormPageContainer><PageHeader title="Editar técnico" eyebrow={technician.name} description="Atualize os dados operacionais, a habilitação e as especialidades." breadcrumbs={[{ label: "Visão geral", href: `/app/${organizationSlug}/dashboard` }, { label: "Técnicos", href: `/app/${organizationSlug}/cadastros/tecnicos` }, { label: technician.name, href: `/app/${organizationSlug}/cadastros/tecnicos/${technicianId}` }, { label: "Editar" }]} /><TechnicianForm organizationSlug={organizationSlug} timezone={context.organization.timezone} technicianId={technicianId} skillOptions={[...optionMap.values()]} initialValues={{ name: technician.name, document: technician.document ?? "", email: technician.email ?? "", phone: technician.phone ?? "", jobTitle: technician.job_title ?? "", baseCity: technician.base_city, baseState: technician.base_state, driverLicenseNumber: technician.driver_license_number ?? "", driverLicenseCategory: technician.driver_license_category ?? "", driverLicenseExpiresAt: technician.driver_license_expires_at ?? "", canDriveCompanyVehicle: technician.can_drive_company_vehicle, notes: technician.notes ?? "", skillAssignments: technician.skills.map((skill) => ({ skillId: skill.skillId, proficiencyLevel: String(skill.proficiencyLevel), isPrimary: skill.isPrimary })) }} /></FormPageContainer>;
}
